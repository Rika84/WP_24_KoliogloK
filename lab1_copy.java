...
public class lab1_copy extends JFrame {
    ...
    private void calculate() {
        try {
            ...
            // Проверка на пустое выражение - Sprawdzanie pustego wyrażenia
            if (expression.isEmpty()) {
                return;  // Если после этого выражение пустое, ничего не делаем - Jeśli po tym wyrażenie jest puste, nic nie robimy
            }

            // Разбиваем выражение на токены - Rozbijamy wyrażenie na tokeny
            String[] tokens = expression.split(" ");

            // Первый проход: обработка умножения и деления - pierwszy krok: obsługa mnożenia i dzielenia
            ArrayList<String> newTokens = new ArrayList<>();  // создаем список для хранения промежуточных результатов - tworzymy listę na wyniki pośrednie
            for (int i = 0; i < tokens.length; i++) {
                if (tokens[i].equals("*") || tokens[i].equals("/")) {
                    double left = Double.parseDouble(newTokens.remove(newTokens.size() - 1));  // левый операнд - lewy operand
                    double right = Double.parseDouble(tokens[i + 1]);  // правый операнд - prawy operand
                    double result = 0;

                    // Проверка типа операции - sprawdzenie typu operacji
                    if (tokens[i].equals("*")) {
                        result = left * right;  // выполняем умножение - wykonujemy mnożenie
                    } else if (tokens[i].equals("/")) {
                        if (right == 0) {
                            textScreen.setText("Error: Division by zero");  // ошибка деления на ноль - błąd dzielenia przez zero
                            return;
                        }
                        result = left / right;  // выполняем деление - wykonujemy dzielenie
                    }
                    newTokens.add(String.valueOf(result));  // добавляем результат в список - dodajemy wynik do listy
                    i++;  // пропускаем следующий элемент, так как он уже обработан - pomijamy kolejny element, ponieważ został już przetworzony
                } else {
                    newTokens.add(tokens[i]);  // добавляем текущий токен в новый список - dodajemy bieżący token do nowej listy
                }
            }

            // Второй проход: обработка сложения и вычитания - drugi krok: obsługa dodawania i odejmowania
            double result = Double.parseDouble(newTokens.get(0));  // первый элемент как начальный результат - pierwszy element jako wynik początkowy
            for (int i = 1; i < newTokens.size(); i++) {
                if (newTokens.get(i).equals("+") || newTokens.get(i).equals("-")) {
                    double nextNum = Double.parseDouble(newTokens.get(i + 1));  // следующий операнд - kolejny operand
                    if (newTokens.get(i).equals("+")) {
                        result += nextNum;  // выполняем сложение - wykonujemy dodawanie
                    } else if (newTokens.get(i).equals("-")) {
                        result -= nextNum;  // выполняем вычитание - wykonujemy odejmowanie
                    }
                    i++;  // пропускаем следующий элемент, так как он уже обработан - pomijamy kolejny element, ponieważ został już przetworzony
                }
            }
            ...
            expression = "";  // очистка выражения при ошибке - czyszczenie wyrażenia przy błędzie
            isResultDisplayed = false;  // сброс флага результата - resetowanie flagi wyniku
        }
    }
